...
int main(){
    ...
    while(1){
      ...
      if(strcmp(buf,"[continents]")==0){
        ...
        cerr<<myNum<<endl;
        for(int i=0;i<namesLen;i++)cerr<<names[i]<<endl;
        ...
        if(armiesmore==0)
          ...
          cLen=0;
          for(i=1;i<=counLen;i++)
            if(counBelong[i]==myNum)cList[cLen++]=i;
          i=rand()%cLen;
          cout<<"placearmies "<<cList[i]<<" 1"<<endl;  //随便找个地方放得了
        }
        ...
        if(myCard<3)cout<<"endtrade"<<endl;
          ...
          for(int i=1;i<=4;i++)if(ctLen[i]>3)ctLen[i]=3;
          for(int i=1;i<=3;i++)
            if(ctLen[i]+ctLen[4]>=3){
              cout<<"trade";
